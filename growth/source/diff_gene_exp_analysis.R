...
growthClass.boxplot <- function(growthClass, varClass,varFeature) {
  colClass <- c("#BC3C29F2", "#0072B5F2")
  plot <- ggplot(data=growthClass, aes_string(x=varClass, y=varFeature, fill=varClass))
...
pdf("growth_plot_classes.pdf",
    width=12, height=8)
growthClass.boxplot(growthClass, "dtClass", "DoublingTime")
growthClass.boxplot(growthClass, "surClass", "Survival")
growthClass.boxplot(growthClass, "slClass", "Slope")
dev.off()
...
expS <- t(CenterANDScal(t(expS))) ## Tranpose twice because we want our row means to be zero
dim(expS)
