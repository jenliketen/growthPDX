...
gsea_N25_c5bp <- doGSEA(geneStat=geneStat, pathwayDB=c5bp, nPerm=10000, ncpus=4)
gsea_N25_kegg <- doGSEA(geneStat=geneStat, pathwayDB=kegg, nPerm=10000, ncpus=4)
gsea_N25_reactome <- doGSEA(geneStat=geneStat, pathwayDB=reactome, nPerm=10000, ncpus=4)

writeWorksheetToFile(file="~/Desktop/growth/data/results/GSEA_doublingTime_N25.xlsx",
                     data=list(gsea_N25_c5bp, gsea_N25_kegg, gsea_N25_reactome), sheet=c("C5 Biological Processes", "KEGG", "Reactome"))

## Get an idea for yourself: 10 most enriched pathways (both directions)
c5bp_up <- gsea_N25_c5bp[gsea_N25_c5bp$dir=="up", "Name"][1:10]
c5bp_dn <- gsea_N25_c5bp[gsea_N25_c5bp$dir=="dn", "Name"][1:10]

kegg_up <- gsea_N25_kegg[gsea_N25_kegg$dir=="up", "Name"][1:10]
kegg_dn <- gsea_N25_kegg[gsea_N25_kegg$dir=="dn", "Name"][1:10]

reactome_up <- gsea_N25_reactome[gsea_N25_reactome$dir=="up", "Name"][1:10]
reactome_dn <- gsea_N25_reactome[gsea_N25_reactome$dir=="dn", "Name"][1:10]

most_enriched_N25 <- list(C5BP=list(Up=c5bp_up, Down=c5bp_dn),
                          KEGG=list(Up=kegg_up, Down=kegg_dn),
                          Reactome=list(Up=reactome_up, Down=reactome_dn))

saveRDS(most_enriched_N25, file="~/Desktop/growth/data/results/most_enriched_N25.Rda")